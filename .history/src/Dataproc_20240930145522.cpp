...
void DataProc::stopLoad(void)
{
    ...
    m_sendCount = 0;
    m_readPro = 0;
    m_BasicAckChannel = 0xFF;
    m_DMRAckChannel =0xFF;
}
...
quint16 DataProc::CheckReceAckOk()
{
    ...
    static quint16 old_channel = 0xff;
    ...
    if(ret_Value == 0)
    {
     ...
     if  ((m_DMRAckChannel == m_sendCount) && (m_BasicAckChannel == m_sendCount)
          &&(m_sendCount != old_channel))
     {
    ...
    if(check_count >= 5) //检测3次还没收到应答 直接退出
    {
...
void DataProc::getChannelBasicParam(const uint8_t* channel_buf)
{
   ...
   int tempNum;
    ...
    //VFA_FREQ
    //qstrncpy((char*)tempString.data(),(char*)(channel_buf+3),4);
    ...
    //VFB_FREQ
    //qstrncpy((char*)tempString.data(),(char*)(channel_buf+7),4);
    ...
    //NAME
   // qstrncpy((char*)tempString.data(),(char*)(channel_buf+13),12);
    QByteArray array;
    array = QByteArray((const char*)(channel_buf+13), 12);//因为buf[]中有`\0`，必须要写上数据长度；否则，数据会直接截断，丢失数据
    tempString = QString(array);
   ...
   // retString = QString(QJsonDocument(m_ChannelParamObject).toJson());
...
void DataProc::onDataReceived(const QByteArray &buff)
{
    ...
    for (i = 0; i < buff.size(); i++)
    {
        ...
        {
            case F_HEAD:
            if((quint16)buff.data()[i] == 0xA5)
            {
               m_headCount ++;
            }
            if(m_headCount >= 4)
            {
                m_stateMachine = F_LEN;
                m_headCount = 0;
              //  qDebug() << "header receieved";
            }
            memset(m_Parse_Buff,0,PROTOCAL_BUFFER_SIZE);
            m_rxLen = 0;
            m_recNum = 0;

            break;
            case F_LEN:
                if((quint16)buff.data()[i] != 0xA5)  //防止粘包
                {
                    m_Parse_Buff[m_recNum] = (quint16)buff.data()[i];
                    m_recNum ++;
                    m_rxLen = (quint16)buff.data()[i] + 1;
                    if((quint16)buff.data()[i] > 0)
                    {
                        m_stateMachine = F_DATA;
                     //   qDebug() << "buff len receieved";
                    }
                }
            break;
            case F_DATA:
                ...
                {
                    m_recNum = 0;
                    //    qDebug() << "data receieved";
                   // qDebug() << "数据接收完成";

                    crcData = CRC16Check(m_Parse_Buff,m_rxLen-2);
                    if(((quint16)(crcData >> 8) ==  m_Parse_Buff[m_rxLen -2])
                        && (quint16)crcData == m_Parse_Buff[m_rxLen -1])  //m_rxLen = m_recNum + 1
                    {
                    //    qDebug() << "CRC 校验完成";
                        Protocal_Get_Command(m_Parse_Buff,m_rxLen-2);
                    //    qDebug() << "CMD is:" << m_command;
                        memset(m_Parse_Buff,0,PROTOCAL_BUFFER_SIZE);
                        m_stateMachine = F_HEAD;
                    }
                    else
                    {
                        m_stateMachine = F_HEAD;
