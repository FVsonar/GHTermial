    ...
    for (i = 0; i < buff.size(); i++)
    {
        ...
        switch (m_stateMachine)
        {
            ...
            case F_LEN: // 处于长度检测状态
                if((quint8)buff.data()[i] != 0xA5)  // 防止数据粘包
                {
                    ...
                    if((quint8)buff.data()[i] > 0) // 如果接收的数据长度大于0
                    {
                     ...
                     //   qDebug() << "buff len receieved"; // 调试输出，显示接收长度接收完成
                    }
                }
            break;
            case F_DATA: // 处于数据接收状态
                ...
                {
                    m_recNum = 0; // 重置接收数据计数器
                   // qDebug() << "data receieved"; // 调试输出，显示数据接收完成
                   // qDebug() << "数据接收完成"; // 调试输出，显示数据接收完成
                    ...
                    {
                    //    qDebug() << "CRC 校验完成"; // 调试输出，显示CRC校验完成
                        Protocal_Get_Command(m_Parse_Buff,m_rxLen-2); // 解析命令
                    //    qDebug() << "CMD is:" << m_command; // 调试输出，显示解析出的命令
                        memset(m_Parse_Buff,0,PROTOCAL_BUFFER_SIZE); // 清空解析缓冲区
                    ...
                    else
                    {
                     ...
                     //   qDebug() << "CRC 校验失败"; // 调试输出，显示CRC校验失败
                    }
