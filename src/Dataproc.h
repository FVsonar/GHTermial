...
#define READ_CHANNEL_MAX_COUNT  1000 //实际数字加一

#define debug(x) qInfo()<<__func__ << "()" << "[" << __LINE__ << "]" << x


// 定义DMR内存结构体，用于存储频道相关配置信息
struct dmr_mem_t
{
    quint16 ch_index;  // 频道索引，用于标识特定的频道
...
private:
    ...
    * 检查接收的数据包是否包含正确的结束标志。
    *
    * @return 返回一个quint16类型的值，表示检查的结果。0表示结束标志错误，1表示结束标志正确。
    */
    quint16 CheckReceAckOk(void);
    ...
    * @param len 数据包中数据的长度。
    */
    void PackData(quint16 cmd, int len);
    ...
    */
    ...
    quint16 m_needRWList[1000];   // 需要读写操作的设备列表，用于存储设备索引
    ...
    quint16 m_readPro;   // 读取进度，表示当前读取操作的完成程度
    QJsonObject m_ChannelParamObject;   // 通道参数对象，用于存储设备通道配置信息
    quint16 m_realRWcount;   // 实际读写操作次数，用于统计读写操作的频率
    quint16 m_DMRAckChannel; // DRM确认通道，用于接收DRM相关的确认消息
    quint16 m_BasicAckChannel;   // 基础确认通道，用于接收基础设备状态确认消息
...
private slots:
    ...
    * @brief 处理发送间隔的时间回调函数
    *
    * 本函数作为定时器的回调函数，用于定期执行数据发送任务。
    * 它主要用于在网络通信中按照一定的时间间隔发送数据或保持连接。
    */
    void onSendInterval();
    ...
    * @brief 处理获取间隔的时间回调函数
    *
    * 本函数用作定时器的回调函数，用于定期执行数据获取任务。
    * 它主要用于在网络通信中按照一定的时间间隔接收数据或检查更新。
    */
    void onGetInterval();
...
public slots:
    ...
    // 解析包含表格数据的JSON数组
    void parseTableJsonArry(QString str);

    // 获取发送计数
    quint16 getSendCount(void);

    // 获取读取计数
    quint16 getReadCount(void);

    // 加载信道参数
    void loadChannelParamater();

    // 从字节中获取JSON数组
    QString getJsonArrayFromByte();

    // 停止加载操作
    void stopLoad();

    // 设置读写列表状态
    void setReadWriteList(quint16 index, bool state);

    // 获取调试模式状态
    quint16 getDebugMode(void)
    {
